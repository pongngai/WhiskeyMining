...
object Regression {
  ...
  def main(args: Array[String]): Unit = {
    ...
    Lm(y,x,List("Freq","Angle","Chord Length","Velocity","Thickness")).summary
    // add an intercept and re-fit
    val X = DenseMatrix.horzcat(
      DenseVector.ones[Double](x.rows).toDenseMatrix.t,x)
    val mod = Lm(y,X,List("(Intercept)","Freq","Angle","Chord Length","Velocity","Thickness"))
    mod.summary
